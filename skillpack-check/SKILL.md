...
See `references/doctor-warning-interpretation.md` for the Windows/Supabase-heavy warning matrix (stale autopilot lock, cycle freshness vs actual outage, effective-date reindex, multi-source drift, and why doctor alone does not prove the current chat model will successfully run a dream/proposal step).
