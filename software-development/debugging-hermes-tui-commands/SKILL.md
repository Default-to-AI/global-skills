...
5. **Desktop/web slash parser treats multiline paste as an empty slash command.** If the user pastes `/goal ...` or another slash command across multiple lines, the frontend parser can split the first line from the payload and surface `empty slash command` even though the command name is valid. Reproduce with a literal newline inside the input, then inspect the frontend slash parsing path (`web/src/lib/slashExec.ts` and any desktop submit/newline handling) before blaming command registration. Practical workaround: keep the slash command itself on one short line, then send the detailed spec as a normal follow-up chat message.
...
- `references/multiline-slash-command-parser.md` — reproduction recipe and operator workaround for desktop/frontend multiline slash-command failures such as `/goal` → `empty slash command`.
...
### Post-Fix Verification Habit (User Preference)

**Always provide concrete verification steps when work completes.** Two preferred modes:

- **Manual steps** — list exact commands, URLs, files to check, and what to look for.
- **Browser verification** — when a web UI is involved (TUI gateway, dashboard, etc.), use `browser_navigate` to open the live page, then `browser_snapshot` or `browser_vision` to show the current state. In the response, tell the user exactly what changed and what to look for visually.

Do not end a task with only a summary. Show the evidence.
