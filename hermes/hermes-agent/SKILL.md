...
metadata:
  hermes:
    tags: [hermes, setup, configuration, multi-agent, spawning, cli, gateway, development, profiles, auth, topology, skill-authoring, governance, reconciliation]
    homepage: https://github.com/NousResearch/hermes-agent
    related_skills: [claude-code, codex, opencode, github-workflow, kanban, obsidian, systematic-debugging, external-coding-agents, planning, code-quality]
---
...
**Extensive session config** (500+ turns, higher tool limits): `references/extensive-session-config.md`
...
### Hermes Venv vs System Python — Package Management

**The Hermes agent runs in its own isolated venv**, located at:
```
~/.hermes/hermes-agent/venv/          (or %LOCALAPPDATA%\hermes\hermes-agent\venv on Windows)
```

This venv is **completely separate** from your system Python (e.g. Python Core at `~/AppData/Local/Python/pythoncore-3.x/` or any `pip`/`python` on PATH).

**Common pitfall:** Running `pip install <package>` installs into the system Python, but Hermes imports from its own venv. The package will appear "already satisfied" when installed globally, but Hermes will still fail with `ModuleNotFoundError` or show warnings like:
```
WARNING plugins.memory.honcho: Honcho background session init failed: honcho-ai is required...
```

**Always use the Hermes venv's pip** for packages Hermes needs:
```bash
# Windows (git-bash / MSYS paths)
/c/Users/Tiger/AppData/Local/hermes/hermes-agent/venv/Scripts/pip3.exe install <package>

# Or via python -m pip (works cross-platform)
/c/Users/Tiger/AppData/Local/hermes/hermes-agent/venv/Scripts/python.exe -m pip install <package>
```

**Verification:** Check the package is visible to Hermes:
```bash
/c/Users/Tiger/AppData/Local/hermes/hermes-agent/venv/Scripts/python.exe -c "import <module>; print('ok')"
```

This applies to **all plugins and memory providers** that declare `pip_dependencies` in their `plugin.yaml` — the setup wizard (`hermes memory setup`, `hermes honcho setup`) attempts auto-install via `uv pip install --python <hermes-venv-python>`, but manual installs must target the venv explicitly.
...
**Path notation:** Both `C:/Users/...` (forward slash) and `C:\Users\...` (backslash) formats are accepted by Hermes tools and most Windows APIs. Prefer forward slashes in code and logs — avoids shell-escaping backslashes in bash. **Note:** Some users may prefer backslash notation (`C:\Users\...`) for personal consistency with Windows Explorer; both formats are functionally equivalent in Hermes tools.
...
## Specialized Operations (Consolidated)

### Skill Authoring (from `hermes-agent-skill-authoring`)

For authoring in-repo skills that ship with Hermes:

- **Two locations**: User-local (`~/.hermes/skills/`) vs in-repo (`skills/<category>/<name>/SKILL.md`)
- **In-repo workflow**: Use `write_file` (not `skill_manage create`), commit to repo
- **Required frontmatter**: `name`, `description` (≤1024 chars, starts with "Use when..."), `version`, `author`, `license`, `metadata.hermes.tags`, `metadata.hermes.related_skills`
- **Size limits**: Full SKILL.md ≤ 100k chars (aim for 8-15k); split into `references/`, `templates/`, `scripts/` if larger
- **Structure**: `# Title` → `## Overview` → `## When to Use` → body → `## Common Pitfalls` → `## Verification Checklist`
- **Pitfalls**: Don't use `skill_manage create` for in-repo; no leading whitespace before `---`; description must start with "Use when..."; commit the edit

### Install Topology Mapping (from `hermes-install-topology`)

Diagnose which Hermes is actually running when paths are confusing:

**Three layers to separate:**
1. **Shared backend state** — `config.yaml`, `.env`, `auth.json`, `state.db`, `logs/`, `profiles/`, `skills/`, runtime repo under `HERMES_HOME`
2. **Desktop shell state** — `%APPDATA%\Hermes` (Electron: `connection.json`, caches, preferences)
3. **Optional source checkouts** — standalone git repos not in use unless executable resolves there

**Windows default paths:**
- `HERMES_HOME` → `%LOCALAPPDATA%\hermes`
- Live runtime repo → `%LOCALAPPDATA%\hermes\hermes-agent`
- CLI launcher → `%LOCALAPPDATA%\hermes\hermes-agent\venv\Scripts\hermes`
- Desktop profile → `%APPDATA%\Hermes`
- Legacy → `%USERPROFILE%\.hermes` (verify, don't assume)

**Investigation:** `command -v hermes` → `hermes config path` → `hermes config env-path` → inspect candidate dirs → check Desktop's `connection.json` → verify update target via live executable's repo remote/branch.

### Profile Auth Management (from `hermes-profile-auth-management`)

Manage credentials across profiles (`~/.hermes/profiles/<name>/auth.json` each):

- **Profile isolation**: Each profile has own `auth.json` (tokens, pool, `active_provider`), `config.yaml` (provider)
- **Credential pool**: Shared source (e.g., `~/.codex/auth.json`) means token reuse breaks all profiles simultaneously
- **Common failures**: `refresh_token_reused` (run `codex` locally first), `usage_limit_reached`, `expired`/401 (re-auth), no credentials for configured provider
- **Re-auth (openai-codex)**: Run `codex` locally → `HERMES_PROFILE=<name> hermes auth` per profile
- **Pitfalls**: Don't `hermes auth` before `codex`; config must match active provider; failed pool entries persist; Windows `codex` CLI must run in Windows terminal for OAuth

### Profile Role Governance (from `hermes-profile-role-governance`)

Update role contracts (`SOUL.md`) across profiles without leaving conflicts:

- **Core rule**: Don't patch one layer and stop — check root `SOUL.md`, profile `SOUL.md`, profile `memories/USER.md`
- **Workflow**: Restate understanding → inspect all layers → create timestamped backup → patch root routing first → patch profile contracts with clear ownership/avoid language → reconcile memories → verify by reading back
- **Recommended split**: Strategist (planning, architecture) / Engineer (implementation, debugging) / Vault (knowledge maintenance) — each with explicit `Avoid` lines
- **Pitfalls**: Tone-only fixes leave profile memories stale; fuzzy delegation rules; strategy/implementation overlap

### Profile Skill Reconciliation (from `hermes-profile-skill-reconciliation`)

Audit and align skill libraries across profiles (`~/.hermes/skills` vs `~/.hermes/profiles/<name>/skills`):

- **Core fact**: Profiles have isolated skill trees; Hermes does not merge automatically
- **Procedure**: Enumerate profiles → compare live FS trees → compute extras/missing vs target (usually `default`) → timestamped backup → delete extras (don't auto-install missing) → verify `extra_vs_target` empty
- **Pitfalls**: Don't call it "inheritance"; don't claim identity unless `missing` also empty; subset result is common; preserve recovery handle
- **Reference**: `references/profile-skill-audit-prune.md`
