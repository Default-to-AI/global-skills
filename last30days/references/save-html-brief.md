...
# IMPORTANT: prefer the configured memory/output root that last30days is already
# using on this machine. On Hermes/Windows installs, `--diagnose` may report a
# `local_writes.path` such as `C:\Users\<user>\Agents\Docs-and-Research\Last30Days`
# even when older docs/examples mention `~/Documents/Last30Days`. If the current
# environment or diagnose output already points at a different write root, keep
# using that root; do NOT hardcode `~/Documents/Last30Days` for a follow-up HTML
# export or raw-save rerun.
